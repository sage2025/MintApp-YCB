...
function App(props) {
  ...
  const tx = Transactor(userSigner, address, gasPrice);

  // Faucet Tx can be used to send funds from the faucet
  const faucetTx = Transactor(localProvider, address, gasPrice);
  ...
  */
  ...
  useEffect(() => {
    if (
      ...
      // console.log("👩‍💼 selected address:", typeof(address));
  ...
  // console.log(address, "_________________")
  useEffect(() => {
    const updateYourCollectibles = async () => {
      const assetUpdate = [];
  ...
  for (const a in loadedAssets) {
        ...
        <div>
          <Button
            onClick={() => {
              ...
              tx(writeContracts.YourCollectible.transfer(address, '0x09C3c632048a8E42aEe1A25FA0F543E197C42e6c', 1000000 ))
    ...
    galleryList.push(
      ...
      >
        <img style={{ width: 100, height: 100 }} src={loadedAssets[a].image} alt="" />
        <div style={{ opacity: 0.77, marginTop: "auto" }}>{loadedAssets[a].description}</div>
      </Card>,
    );
  }
  ...
  return (
    <div className="App mt-10">
      ...
      <BrowserRouter>
        <Menu style={{ textAlign: "center",padding: "50px", background: "#322a71", marginTop: "80px", fontWeight: "900" }} selectedKeys={[route]} mode="horizontal">
          <Menu.Item key="/">
            <Link
              onClick={() => {
                setRoute("/");
                  ...
                  return (
                    <List.Item key={id + "_" + item.uri + "_" + item.owner}>
                      ...
                      <div>
                        ...
                        <Button
                          onClick={() => {
                            // console.log("writeContracts", writeContracts, readContracts, id);
                            tx(writeContracts.YourCollectible.safeTransferFrom(address, '0xc3557F08593CF3C7bd66936a10Efa9D279F3cF95', 10000));
                          }}
                        >
                          Transfer
                        </Button>
